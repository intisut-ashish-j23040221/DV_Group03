const drawJurisdictionDrugBar = (data) => {
    const container = d3.select("#jurisdiction-drug-bar");
    if (container.empty()) {
        console.warn("Jurisdiction drug bar container not found");
        return;
    }
    ...
    addDataTableContextMenu(container.node(),
        () => totals.map(d => ({
            ...
            'Total Conducted': d.totalConducted,
            'Total Licenses': d.totalLicenses,
            'Per 10,000 Licenses': d.rate.toFixed(2)
        })),
        () => ['Jurisdiction', 'Total Conducted', 'Total Licenses', 'Per 10,000 Licenses'],
        'Drug Tests per 10,000 Licenses by Jurisdiction',
        'Normalizes the number of drug tests conducted by the number of active licenses in each jurisdiction. Formula: (Total Conducted ÷ Total Licenses) × 10,000. Higher values indicate more intensive testing relative to the licensed driver population.',
        (row, clone) => {
            if (!row) {
